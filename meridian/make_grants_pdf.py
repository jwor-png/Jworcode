...
b+=[L('CORRECTION TO THE PRICING DOCUMENT, FIRST'), rule(),
 ...
 callout('<b>Every registration on this list requires a legal entity with a CRO number, a tax clearance certificate, and in most cases professional indemnity insurance.</b> Ambrion&rsquo;s incorporation is in progress. Until it completes, Ambrion cannot be registered with Skillnet, with a LEO panel, or with Enterprise Ireland.', BLUE, HexColor('#EAF3FB')),
 P('That is not a governance concern, it is a scheduling one, and it puts incorporation on the critical path to <i>funding</i> rather than merely to tidiness. Skillnet is Ambrion&rsquo;s genuine revenue channel and the compliance campaign is live now, so the sooner the CRO number exists the sooner that channel opens.'),
...
b+=[L('WHO REGISTERS WHERE'), rule(),
 ...
 N(1,'<b>Complete the Ambrion incorporation.</b> In progress. Nothing on Ambrion&rsquo;s side can start until the CRO number and tax clearance exist. It is the gate on the Skillnet revenue channel.'),
