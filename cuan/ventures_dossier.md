# Ventures Dossier — John's Live Projects

## System Architecture Note
There are **two intelligence systems**:
- **Cuan** — John's Personal Intelligence Orchestration system. Holds John completely (personal and commercial, because the two are inseparable).
- **Meridian Intelligence** — the separate commercial intelligence system. Jurgen (legal AI) sits inside Meridian.
...
### Open Loops -- portfolio structure, added 5 August 2026

| Priority | Action | Notes |
|---|---|---|
| HIGH | ODIN: settle and record John's position | Confirmed by John 16 Aug that he will have the conversation with Shane. ODIN is the only one of the four vehicles with no recorded ownership: Meridian 70/30, Velocity 30/70, Ambrion 30 of 60/30/10 are all documented; ODIN shows a role only, "Head of Investments", from the July business plan sent via Carl Webb to Simone and Tench Coxe. ODIN is also the asset-producing vehicle (the Partner Design Studio is an ODIN asset) and the one Shane is planning to scale. Settle before external capital is in the room, not after. |
...
**Part 1 -- Competitor intelligence (built from the training session itself). Cuan holds 2 of 3:**
1. **Competitor AI Analysis** (9 pages, verification chain against 70 sources, checked claim-by-claim by a second model: 72% verified cleanly, 15% partial/rewritten, 13% corrected or removed -- most material fix was the Do it Best/United Hardware US merger date, off by over a year in the draft). Maps the US (Lowe's Mylow, Home Depot Magic Apron, Ace Hardware's Hey ARMA in 2,300+ stores), Australia/NZ (Bunnings, Mitre 10, Metcash) and UK/Ireland players. Finding: United Hardware currently has **no public AI footprint at all** -- no chatbot, no AI search, no personalisation, no forecasting on its Aphix-built platform -- read as open whitespace, not a lost race.
2. **Executive Summary** (2 pages, commercial-in-confidence). The distilled version: proof points (Toolstation 95% fewer failed searches, Kingfisher +37%/~£80m uplift, Bunnings 2x conversion, two-thirds of UK merchants already personalising). Ranks three moves by leverage: (1) member-store AI assistant -- highest leverage, the one thing no single member can fund alone; (2) AI product search; (3) product-data enrichment and demand forecasting. Two cautions: do not follow Bunnings into facial recognition (found unlawful in Australia); any customer assistant must hard-refer licensed electrical/plumbing work from day one. Recommended next step: commission a scoped pilot of the member-store assistant as lead build, AI product search as fast follow.
3. **Evidence Pack** (1 page, the numbers behind the case, sourced). Hard proof points across four categories: buying/purchasing (Bunnings' Buddy assistant 2x online conversion; Kingfisher AI recommendations +37% YoY, ~£80m in one half; McKinsey values gen AI to retailers at $240bn-$390bn); personalisation (Wickes £7m incremental revenue from one ML engine; two-thirds of UK builders' merchants already personalising); ease of transaction (Toolstation: failed searches 2% to 0.1%, AI now handles ~68% of logged-in queries, staff turnover 40% to 18%; production assistants now build in 4-6 weeks); collections and fulfilment (US click-and-collect data, directional for Ireland; AI forecasting cuts errors 30-50%, enrichment lifts catalogue completeness 52% to 93%).

**All six documents of the proposal are now confirmed and filed.**
...
- **Planning status (confirmed from Bronagh Clarke emails, 10 and 23 June 2026):** P.A. Ref **25-60781**, applicant Casana Developments Limited, Meath Co Co (planner: Gerard Kellett, Senior Executive Planner). FI response submitted 29 May; revised public notices lodged 10 June (press notice in the Star same day; three site notices on the Duleek Road, monitored by Michael Heather, all in place 22 June). **Objector submissions close Tuesday 14 July 2026. Decision due on or before Tuesday 4 August 2026** (8 weeks from 10 June; longer period because the application carries an NIS). As of 23 June, no submissions registered. Favourable outcome expected; sale proceeds to completion as soon as possible thereafter.
- **Watch window (8-14 July):** the R132 traffic-relief consultation on 8 July puts detailed route options into the public domain six days before the Old Mill submission deadline on the 14th. If anything in the four options could be used by an objector, the team has that window to see it coming. Myki's read-back from the 8th covers this.
