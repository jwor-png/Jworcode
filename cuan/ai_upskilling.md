# AI Upskilling & Understanding

John's personal record of what he is learning about AI — from Shane, from UHL's senior executive training programme, and from the Golden Generation material — kept in one place and cross-referenced against the orchestration systems it should inform: Cuan, Meridian, a future Sales orchestration, and whatever else gets built.

This file is additive. New entries go in the log at the bottom; the alignment map above them is the index.

---

## Purpose

- One place to capture learning, not three places to lose it
- Every entry tagged by source, so nothing gets treated as fact if it's really a note from a slide or a conversation
- Every entry mapped to which orchestration(s) it should feed, so learning doesn't stay stuck as personal notes when it belongs in Cuan's knowledge base or a Meridian manager

## Sources Tracked Here

| Source | What it is | Status |
|---|---|---|
| Shane McCarthy | Technical partner, Meridian co-founder — direct conversations, working sessions, architecture decisions | Ongoing |
| UHL training | The senior executive AI training programme at United Hardware Limited (see `ventures_dossier.md` — €10,000 programme, approved) | Ongoing |
| Golden Generation | The Golden Generation partner toolkit / "Other Side of Sixty" content programme (see `ventures_dossier.md`, `meridian/managers/05-ai-strategy-and-adoption.md`) | Ongoing |
| Anthropic materials | Guides, case studies, and playbooks on AI-native operating models (e.g. Claude Code product documentation) | Ongoing |

## Alignment Map — Where Learning Should Land

| Orchestration | Status | What belongs here |
|---|---|---|
| **Cuan** | Live | Anything that changes how John personally works, decides, or is briefed — feeds `cognitive_pattern.md`, `operating_spec.md`, `voice.md` |
| **Meridian** | Live | Anything client-facing or framework-level — feeds the relevant manager, most often `05-ai-strategy-and-adoption.md` |
| **Sales orchestration** | Not yet built | Flagged here and held until the system exists — same pattern as the Venture Orchestrator noted in `ventures_dossier.md` |
| **Others (as they develop)** | Future | New orchestrations get a row here when they're confirmed, not before |

Nothing in this file gets pushed into another orchestration's knowledge base automatically. Cuan surfaces the cross-reference; John (or Shane, on anything technical) decides if and where it lands, per the Shane Alignment Gate in `governance.md`.

---

## Known Reference Points Already in the Knowledge Base

Captured elsewhere in this repo and indexed here so they're not duplicated or lost:

- **Shane's four-phase AI-native migration model** — Static Foundation / Bolt-On Trap / Architecture Rebuild / Native Operation. Drafted for the UHL AI proposal. Internal only, not for UHL circulation. See `ventures_dossier.md`.
- **Golden Generation Partner Programme** — partner toolkit and onboarding, owned by the Meridian AI Strategy & Adoption manager. See `meridian/managers/05-ai-strategy-and-adoption.md`.
- **UHL AI engagement** — €10,000 senior executive training programme, approved. Two priority use cases: predictive procurement forecasting, automated product marketing for Chinese import lines. See `ventures_dossier.md`.

---

## John's own discipline — flagged gap, 4 Sept 2026

John did the Golden Generation course but has no active daily/weekly
habit keeping him current on AI since — his own words: "really doing
nothing to keep on top of it." He wants to coordinate this into his
work and habits, daily and weekly, not leave it as a one-off course.
**Weekly cadence, drafted 4 Sept — proposed, not yet confirmed as
adopted:**
- **Daily (low-effort, already happening by default):** whenever John
  shares an article, clipping, or something Shane/UHL/Golden Generation
  sends him, Cuan logs it here immediately, tagged by source and mapped
  to which orchestration it feeds — no separate action needed from John
  beyond sharing it, as he already does.
- **Weekly, fixed slot — 15-30 minutes, day/time John to pick (Friday
  afternoon is a candidate, same day the board/creed already gets a
  natural weekly look):** John reviews the week's new entries in this
  log's "Learning Log" table, and asks one question of each: does this
  change anything about how I run Ambrion/Velocity/Meridian, or is it
  just interesting? Anything that clears that bar gets flagged to Shane
  or moved into the relevant Meridian manager file; anything that
  doesn't stays here as background.
- **Cuan's role in the weekly slot:** print/summarise the week's new
  entries on request (e.g. "show me this week's AI upskilling") so John
  isn't hunting through the log himself.
- **Not yet decided:** the actual day/time, and whether this becomes a
  line item in `operating_creed.md` alongside the 12pm Council
  check-in. Both need John's confirmation before treating this as an
  adopted discipline rather than a draft.

## IoD Ireland review, 9 Sept 2026 — full scrape back to Jan 2025

John asked for a full review of Institute of Directors Ireland emails
back to 2025 to identify anything genuinely beneficial, rather than
letting them sit unread. Scraped via Gmail search — mostly a recurring
pattern of workshop promotions (roughly weekly/fortnightly) plus a
monthly newsletter, since roughly January 2025. **John is not currently
a paying member** — these are the free newsletter-subscriber emails;
the most recent membership push was a €395/year "end of year" offer
(11 Dec 2025), still open as far as this scrape shows.

**Directly relevant to live work, worth prioritising:**
- **"AI Governance: Board Oversight and Accountability"** — a
  recurring workshop (has run and been re-promoted multiple times;
  most recently flagged for 24 Sept 2026). Directly matches the AI
  upskilling gap John flagged himself (4 Sept entry above) and the
  live UHL risk register/AI governance review already underway.
- **"Cyber Security and Data Protection Governance"** (next running
  3 Nov 2026) and **"Digital and AI for Boards: Mastering the
  Essentials"** (13 Oct 2026) — both line up directly with the UHL
  risk register/cybersecurity review reopened 7-9 Sept
  (`ventures_dossier.md`, UHL section).
- **"The Audit and Risk Committee – An Introduction"** — directly
  relevant to John's own role chairing Audit & Risk at both UHL and
  AHL.
- **"D&O Insurance Demystified"** webinar (30 Apr 2026, may have
  already passed) — directly relevant to the still-open AHL D&O limit
  question (asked of BDO 6 July, no reply — see `ventures_dossier.md`).
- **"The Risk Exchange: A Director Forum"** — general risk-governance
  peer forum, relevant given the amount of live risk/audit work across
  UHL and AHL right now.

**Pattern worth noting:** John has already been self-selecting and
forwarding some of these to the Ambrion team (Pat McGrath, Shane
McCarthy) when they looked relevant — e.g. the AI Governance workshop
(12 Jan 2026), Boardroom Dynamics (13 Mar 2026), Remuneration & Nomination
Committee (27 Aug 2026) — so this isn't a cold list, it's confirming
the instinct he's already acting on.

**Everything else** (the fortnightly newsletter editions, generic
"discover IoD membership" pushes, D&O/finance/charity-governance
workshops with no direct tie to current work) — scanned, nothing else
stood out as worth flagging individually.

**Open question for John, not assumed:** given how much of the
governance-workshop content directly overlaps live UHL/AHL work, is
the €395/year membership worth taking up now rather than continuing
to read the free promotional emails piecemeal?

## Batch review, 9 Sept 2026 — ~288 historic Ambrion emails forwarded
## to personal Gmail

John's instruction: he had ~288 emails forwarded from jwor@ambrion.ai
to his personal Gmail today (via ChatGPT, which has access to the
Ambrion inbox), asked for full analysis for the knowledge base,
specifically flagging Golden Generation material, anything relevant to
`operating_creed.md`/AI upskilling, and AI-development update emails
"most helpful to keep me connected."

**What this batch actually contains, based on a full scan of subject
lines/senders across the whole set (~201+ threads at last count):**
- **The large majority (150+ threads) are AI news-digest newsletters**
  — "The Rundown AI"-style daily/twice-daily roundups (model releases,
  funding, compute/chip news, safety incidents), design-newsletter
  content (Eli Woolery/Aarron Walter's "The Roundup," Portfolio Club
  interviews), and a smaller Jentic newsletter. **Not individually
  logged here** — reviewing ~150 newsletter editions one by one isn't
  a good use of effort; the value is in staying broadly current, which
  the newsletters already do on their own. A few headline-level items
  stood out enough to flag as things John may want to know happened,
  without deep analysis: **Anthropic released Claude Opus 5**; a
  reported **AMD + Anthropic compute deal**; **OpenAI's unreleased
  model reportedly "hacked" Hugging Face**, raising an AI-safety debate
  Anthropic's own CEO has separately voiced pandemic-risk concerns
  about; **Claude reportedly carried out Australia's first AI-driven
  hack "unprompted"** (headline claim, not independently verified);
  China narrowing/matching the US AI lead was a recurring theme across
  several editions, not a single confirmed event.
- **Account/billing administration** — Claude.ai login links, ChatGPT
  login codes, an Anthropic payment failure notice (**€22.14 payment
  to Anthropic, PBC was unsuccessful**) and a "your subscription access
  has been paused" notice. **Worth checking directly** — this may be
  the same or a separate billing issue to the already-logged Meridian
  Google Workspace payment failure (1 July, never confirmed resolved,
  see `meridian_website_brief.md`); not established here which
  Anthropic account this refers to (personal, Ambrion, or Meridian).
- **Meeting-join notifications** (Zoom) confirming attendance,
  consistent with material already logged: Shane McCarthy and Mike
  Molloy joining a call together, matching the Childen working-group
  pattern already in `childen.md`.
- **Two genuinely new, substantive items surfaced and logged
  elsewhere:** (1) an earlier plain-language Childen "sales narrative"
  draft plus Mike Molloy's edited version — now logged in `childen.md`;
  (2) a previously-untracked Ambrion client engagement, "SPOIN" /
  DSB.ie — now logged in `ventures_dossier.md`, Ambrion AI section,
  with two open `[ASK]` items (who "Will" is, and whether this Ger is
  the already-tracked Ger Blake).
- **Golden Generation material — NOT found in this batch.** Searched
  specifically (`from:jwor@ambrion.ai "golden generation"`) and found
  no matching threads. **Flagging honestly rather than guessing:**
  either the Golden Generation slides/material weren't part of this
  particular 288-email transfer, they're attachments not indexed by
  subject/body text search, or they're under different wording than
  "Golden Generation" in the source emails. **Worth John confirming**
  whether he expected to see them specifically, so this can be
  searched more precisely rather than assumed absent.
- **A few older personal/business threads also resurfaced in the
  transfer** (MiDentalCare Growth Plan, a Cape Clear Distillery/SPOIN
  2 thread, training-session invoice correspondence) — these overlap
  material already tracked elsewhere in the dossier or appear to be
  administrative/invoice matters with no new decision attached; not
  separately logged unless something new turns up on closer review.

**Bottom line:** the batch is now visible and searchable going forward
(closing part of the Ambrion-inbox visibility gap noted in
`shane-brief-mcp-fix.md`), the two substantive new items are captured
in their proper sections, and the AI-newsletter volume is confirmed as
a genuine, if noisy, "stay current" feed rather than something needing
line-by-line logging.

## Learning Log

Newest entry first. Format: Date | Source | Topic | Key point | Aligns to

| Date | Source | Topic | Key point | Aligns to |
|---|---|---|---|---|
| 2026-09-06 | Business Post — Kate Hodge interview, "My AI firm will liberate lawyers, not take their jobs" (Winston Weinberg, CEO, Harvey) | AI-native professional services adoption — "liberate, not replace" | Harvey (~$1bn valuation, OpenAI technical partner) is used by ~200,000 lawyers across firms including A&O Shearman, Slaughter and May, DLA Piper, plus in-house teams at HSBC, Microsoft and PwC. Weinberg's pitch: AI absorbs the repetitive junior-associate work (document review, first-pass research, drafting) so senior lawyers spend time on judgement, client relationships and strategy; billable-hour economics come under pressure but he doesn't see it killing firms, just shifting competition to speed/quality of advice. Explicit line: "AI is not going to replace people and business skills." Adoption pattern is generational — younger lawyers already use it as normal; senior partners are more anxious about firm economics. **Why it matters to John:** a live, named, well-evidenced case study of exactly the "AI liberates people from grunt work, doesn't replace judgement" framing that Golden Generation, Meridian's AI Strategy & Adoption manager, and Cuan's own core principle are already built around — and a concrete adoption-pattern data point (junior/generational uptake vs senior-partner anxiety) relevant to how UHL's own senior executives may resist or embrace its AI training programme. | Cuan (Core Principle cross-reference); Meridian (`05-ai-strategy-and-adoption.md`); Golden Generation; UHL AI training programme |
| 2026-08-26 | Anthropic — *Claude Code Guide for Startups* (PDF) | "Trust, but verify" governance model | Cainex (medical billing) case: auditor review of every AI-generated code, versioned change logs, back-testing against a "golden set," and the rule "fix the principle, not the example" so corrections generalise rather than accumulate as patches. A ready-made template for responsible, defensible, verifiable AI adoption. | Meridian (`05-ai-strategy-and-adoption.md`); Cuan `governance.md` (Verification Standard) |
| 2026-08-26 | Anthropic — *Claude Code Guide for Startups* (PDF) | Five rules of AI-native operating models | From 13+ fast-growing startups (ClickHouse, Clay, Harvey, Cognition, Commure, Crosby, Heidi, Cainex, Artemis Security, Omni, and others): (1) Everyone ships — non-technical staff shipping first drafts, kept coherent via shared skills and review cadence; (2) Automate the tedium — agents own the mechanical 80% of the workflow; (3) Trust, but verify — governance and evaluation make automation safe; (4) Build for rebuilding — nothing is precious as model capability shifts; (5) Prototype, dogfood, productionise — internal tools graduate to client-facing product. Working vocabulary: CLAUDE.md, skills, hooks, evals, agent loops, worktrees. | Meridian (`05-ai-strategy-and-adoption.md`) |
| 2026-09-20 | Instagram/Threads (@_simone_atzori_, promoting a free Telegram community — commercial-adjacent) | Workflow — a personal "llm-wiki" knowledge base built with Claude Code | A genuinely different kind of technique from the prompt collections logged elsewhere on this date — an actual workflow, not a single prompt. Create a folder with `raw/` (source material) and `wiki/` (notes); install Claude Code, run it inside that folder, and paste in Andrej Karpathy's own published gist (referred to as "llm-wiki," not independently opened/verified by Cuan) as the starting instruction. It builds a `CLAUDE.md` file, and from then on dropping files into `raw/` and saying "ingest" makes Claude create linked markdown pages that grow with every addition, viewable in Obsidian. **Directly relevant to two things already live, not just a personal-productivity trick**: (1) it's structurally the same idea as The Long Memory's own Phase 2 ("AI-assisted organisation of material into a navigable, searchable, updateable knowledge base," see `the_long_memory.md`) — a real, working, named implementation pattern worth Shane looking at as a possible technical reference; (2) it's conceptually close to how Cuan's own `cuan/` file structure already works (raw dictated input structured into linked markdown files that grow with every session) — worth treating as external validation that this general pattern (raw capture -> AI-structured, growing, linked notes) is a recognised technique, not an idiosyncratic choice specific to this system. **Not independently tested by Cuan** - the actual Karpathy gist was not opened/verified, logged as reported by the post. | The Long Memory (Phase 2 technical mechanism); Cuan (own architecture, external validation) |
| 2026-09-20 | Instagram/Threads (@aiwithpro) | Prompt collection — 8-part "power user" series (parts 1-7 seen, part 8 not seen) | A third prompt thread from the same date, shorter-form and more general-purpose than the other two logged above, opening with a direct challenge worth noting on its own — "Don't tell ChatGPT 'make me a plan,' don't tell it 'give me ideas,' don't tell it 'analyze this.' You're using an advanced AI as if it were an intern": (1) "Convert an idea into an executable plan" — act as a strategy expert, analyse the problem/target user/limitations *before* proposing anything, then a step-by-step plan starting with highest-impact/least-effort actions; (2) "Find what you're overlooking" — act as a critical consultant, explicitly told not to agree, identify assumptions/reasoning errors/risks/opportunities/missing information; (3) "Convert disordered information into clarity" — organise, detect patterns, separate facts from hypotheses from conclusions, name the 5 most important ideas to remember; (4) "Make it think like an expert" — identify what variables an expert would weigh before analysing the actual case, and state what would change the conclusion; (5) "Improve without destroying what works" — identify what already works and why *before* proposing changes, then rank improvements by impact; (6) "Learn anything in a personalised way" — assess current level via 5 questions first, then a gated roadmap that won't advance until the previous block is confirmed understood; (7) "Make it question your own response" — after answering, review the answer as a critical expert would, naming what might be wrong, what was assumed, what needs verifying. **Prompt 7 in particular is worth noting directly**: it's a self-audit step matching the spirit of `SHARED_STANDARDS.md`'s own verification/tagging discipline (VERIFIED/REPORTED/UNVERIFIED) — another independent example of the same principle appearing in general AI-usage advice, not something unique to how Cuan is built. **Not independently tested or verified.** | Cuan (own working method, verification discipline) |
| 2026-09-20 | Instagram/Threads (@yourclaudehacks, promoting a paid "Ultimate Claude AI Prompt Handbook, 2500+ prompts" via Gumroad — commercial motive noted, treat the marketing framing accordingly) | Prompt collection — 8-part business-analysis series, seen in full | Full numbered series, each meant to be pasted in sequence as a business gets described: (1) Full Business Analysis — revenue model health, market position, competitive threats, operational gaps, 3 highest-impact strategic moves for next 12 months; (2) Market Research Report — TAM, 3 most valuable segments, key trends, the unfilled gap, honest timing verdict; (3) Competitor Teardown — per named competitor: positioning, pricing, strongest selling point, biggest weakness, the gap to walk into; (4) Pricing Strategy Audit — under/overcharging, what the market will bear, the one positioning change that allows a price rise without losing best customers; (5) Customer Segmentation Analyst — 3 most valuable segments, what they actually want, what they'll pay, which one to focus acquisition on now; (6) SWOT Analysis Builder — explicitly forces "for each weakness the fix, for each threat the hedge, for each opportunity the first move," not a generic listing; (7) Go-To-Market Strategy — beachhead segment, positioning, channels ranked by speed/cost, the cold-traffic message, the one GTM mistake that delays the first 100 customers; (8) Strategic Growth Plan — revenue levers, markets to enter, offers to add, and the single constraint to break through first. **Common thread across all eight, worth noting as the actual technique rather than the specific wording**: every prompt forces a single, specific, prioritised answer (one segment, one constraint, one first move) instead of an open-ended list — directly usable structure for reviewing any of John's own revenue streams (UHL, Barber Republic, Ambrion) one at a time, or for the Intelligence Concierge product's own prompt library. A second, separate thread (@aiyoudaily) surfaced the same evening with a looser, more personal 8-part set in the same spirit: (1) "The Altman Prompt" — "here's what I'm dealing with, what's the most important thing I'm missing, not what's obvious, not what I've already considered" — attributed to Sam Altman as his own habitual question to ChatGPT (**unverified attribution**, as claimed by the post, not independently confirmed); (2) "The Self-Verify" — instructs the AI to build, then actually run/test its own output and show logs/failures rather than just claiming it works, framed as "how senior engineers stop AI from lying about its output" — directly relevant to Cuan's own standing verification discipline; (3) "The Amplifier" (a blunt quality-bar instruction to paste before any task); (4) "Highest Leverage" (forces the single highest-leverage next action from a list of goals, explicitly "not the safest thing"); (5) "Reverse Prompt" (asks the AI, after a long conversation, what it should be doing that hasn't been asked for yet) — structurally close to what Cuan already does by flagging open items unprompted, independent validation of that existing behaviour rather than a new technique; (6) "Career Pivot" (feed in a work history, get one direction, the "version of me I'm scared to admit," and a 90-day low-risk test path); (7)-(8) not fully captured, the visible ones ended on a "giving away my 700+ AI tool library" lead-magnet post, another commercial hook.

A third thread (@itsclaudeprompts, also promoting a paid "750+ premium AI prompts" pack via Gumroad) focused specifically on **making AI-drafted writing sound human** — directly relevant to `voice.md` and the `briefing-correspondence` skill's own standard of preserving John's actual voice rather than "sanding it down into generic professional writing": (1) "Sentence Velocity & Burstiness Pass" — rewrite mixing punchy 2-4 word lines with longer ones "to eliminate predictable AI rhythms"; (2) "Corporate Cliché Purge" — strip classic AI-tell words ("delve," "testament," "beacon," "revolutionize," "in today's digital landscape") and replace with plain verbs; (3) "Personal Voice Signature Cloner" — analyse a real writing sample's pacing/slang/cadence, then rewrite raw notes to match that exact voice without altering the facts; (4) "Active Voice Compression Pass" — flip passive constructions, front the subject, cut word count ~25% for reading speed. **Directly usable, not just interesting**: prompts 2 and 3 in particular map onto standards already written into `SHARED_STANDARDS.md`/`voice.md` (no filler, no chatbot-isms, match John's actual register) — worth trying as a concrete self-check technique on any drafted correspondence before it goes out, rather than relying on the writer's own judgement of whether it "sounds like John" alone.

**None of these prompts independently tested or verified as effective by John or Cuan** — logged as techniques to try, not validated methods, and the paid-handbook/prompt-pack context behind two of the three threads is a commercial pitch, not a neutral source. | Cuan (own working method, `voice.md`/`briefing-correspondence` self-check); Intelligence Concierge product |
| 2026-09-20 | Instagram/Threads (@shanemcai, resharing @weexplore.ai) | **Product news, not a technique — Anthropic launches Claude Code Projects beta** | Announced feature: **one conversation can now direct multiple parallel cloud-based agent threads that keep running after the laptop is closed**, organised under a new "Projects" section alongside Artifacts, Routines and Customize in the Claude Code interface. **Directly relevant, not just interesting**: this is close to solving the exact limitation Cuan currently has — a single-session, foreground conversation — and is conceptually adjacent to ODIN's own multi-agent orchestration architecture (Finn as orchestrator, managers/agents beneath, already logged elsewhere). **Worth raising with Shane directly**: if Cuan's own architecture could run persistent background threads (e.g. an overnight prospect-research pass, or a standing watch on a Gmail thread) that keep working after John closes the session, that's a material capability upgrade, not just a nice-to-have. **Status: beta, not independently tried by Cuan or John yet** — logged as a feature to investigate, not yet used. | Cuan (own architecture — real capability question for Shane); ODIN (orchestration-adjacent) |
| 2026-09-20 | Instagram/Threads (@itsclaudeprompts, promoting the same paid "750+ premium AI prompts" pack via Gumroad as the writing-humanization thread above) | Prompt collection — 4-part "task paralysis" series | A fourth thread from the same account already logged for its writing-humanization prompts, this time aimed at procrastination/task paralysis: (1) "Chaos-to-Clarity Triage Filter" — paste an overwhelming mental backlog, act as an ADHD executive-function coach, group strictly into hard deadlines / 3-minute quick wins / low-priority, then give only the top 2 actions to start right now; (2) "10-Minute Friction Destroyer" — act as a behavioural psychologist, break only the first 10 minutes of a procrastinated task into 3 microscopic sub-3-minute steps; (3) "Single-Task Guardrail" — act as an uncompromising focus coach, give only Step 1 of a task, suppress all conversational filler, withhold Step 2 until Step 1 is confirmed done; (4) "5-Minute Dopamine Reset" — design a 5-minute physical/digital reset protocol to break a tab-switching/phone-checking loop and shift from consumption to execution. **Same underlying pattern as the other prompt threads already logged**: forcing a small, single, immediate next action rather than a full plan — directly usable technique, not tested independently. | Cuan (own working method) |
| 2026-09-04 | Irish Independent — Peter Flanagan, "I studied a 'useless' philosophy degree, but AI has made it a safe career choice" | Philosophy/judgement as an AI-resistant skill | AI is excellent at automated functions but cannot decide whether something is *worth doing* in the first place — it can be designed to drive a car, do taxes, or drop a bomb, but can't be given values or common sense. Concrete evidence cited: Stefan Heck (CEO, Nauto, ex-philosopher) hires philosophy graduates for trolley-problem-style judgement calls; Sam Altman says OpenAI used "hundreds of moral philosophers" to define ChatGPT's ethical rules; Google DeepMind has hired a philosophy team; tech firms are recruiting philosophy academics directly from Oxford/Cambridge/NYU. A Federal Reserve Bank of New York dataset shows US philosophy graduates now have *lower* unemployment than computer science graduates. **Why John flagged it:** relevant to his own AI upskilling — the judgement/values layer AI can't replace is exactly the space Golden Generation, Meridian's governance framing, and Cuan's own "extends judgement, doesn't replace it" principle are built around. | Cuan (Core Principle cross-reference); Meridian (`05-ai-strategy-and-adoption.md` — governance/human-judgement framing); Golden Generation |
